...
local WIDGET_DIR = HOME_DIR .. '/.config/awesome/themes/custom/widgets/'
local ICONS_DIR = WIDGET_DIR .. 'icons/'

local net_speed_widget = {}

local function convert_to_h(bytes, color)
    local speed
    local dim
    local bits = bytes * 8
    if bits < 1000 then
    ...
    return string.format("<b><span foreground='%s'>%d</span></b> %s", color, math.floor(speed + 0.5), dim)
end
...
local function worker(user_args)
    ...
    net_speed_widget = wibox.widget {
	{
        ...
        set_rx_text = function(self, new_rx_speed)
            self:get_children_by_id('rx_speed')[1]:set_markup(new_rx_speed)--set_text(tostring(new_rx_speed))
        end,
        set_tx_text = function(self, new_tx_speed)
            self:get_children_by_id('tx_speed')[1]:set_markup(new_tx_speed)--set_text(tostring(new_tx_speed))
        end
    }
    ...
    local update_widget = function(widget, stdout)
        ...
        widget:set_rx_text(convert_to_h(speed_rx, "#FFFF00"))
        widget:set_tx_text(convert_to_h(speed_tx, "#FF00FF"))
