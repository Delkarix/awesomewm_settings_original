...
-- beautiful.init(gears.filesystem.get_themes_dir() .. "default/theme.lua")i
local THEME_NAME = "custom"

local themes_dir = os.getenv("HOME") .. "/.config/awesome/themes/" .. THEME_NAME .. "/"
local theme_path = themes_dir .. "theme.lua"
local widgets_path = "themes/" .. THEME_NAME .. "/widgets"
beautiful.init(theme_path)

-- Load widgets
local fs_widget = require(widgets_path .. "/fs")
local ram_widget = require(widgets_path .. "/ram")
local cpu_widget = require(widgets_path .. "/cpu")
local net_widget = require(widgets_path .. "/net")
...
-- Application menu
appmenu = require("themes/" .. THEME_NAME .. "/appmenu")

-- Autorandr menu
autorandr = {
    {"small", "autorandr small"},
    {"big", "autorandr big"},
    {"dual_l", "autorandr dual_l"},
    {"dual_r", "autorandr dual_r"}
}
...
mytextclock = wibox.widget.textclock("<b><span foreground='#ff0000'><i>%A</i></span>, <span foreground='#ffff00'>%B %e %G</span>, <span foreground='#00ff00'>%l:%M %p</span></b> ")
...
awful.screen.connect_for_each_screen(function(s)
	...
	--[[style = {
		...
		shape_border_color_focus = "#7F7F7F",
		--spacing = -12,
	...
	{ -- Right widgets
	    ...
	    fs_widget(),
	    separator,
	    battery_widget{show_current_level = true, display_notification = true, notification_position = "bottom_right", warning_message_position = "top_right"},
            separator,
	    mytextclock,
            --wibox.widget.textbox("<b>HELLO TEST</b>"),
            separator,
	    s.mylayoutbox,
        }
    }
...
globalkeys = gears.table.join(
    ...
    awful.key({ modkey, "Control", "Shift" }, "Right", function () awful.screen.focus_relative( 1) end,
              {description = "focus the next screen", group = "screen"}),
    awful.key({ modkey, "Control", "Shift" }, "Left", function () awful.screen.focus_relative(-1) end,
    ...
    -- I don't know what most of these do so I'll just disable them for the mean time.
    --[[
...
clientkeys = gears.table.join(
    awful.key({ modkey, "Shift" }, "Up",  function (c) c:relative_move( 0,  0, 0, -20) end,
              {description = "resize window height-20", group = "client"}),
    ...
    awful.key({ modkey, "Shift" }, "Left",  function (c) c:relative_move( 0,  0, -20, 0) end,
              {description = "resize window width-20", group = "client"}),
...
-- {{{ Scripting
prt = function(...) naughty.notify{text=table.concat({...}, '\t'), fg="#FF0000", bg="#00FF00"} end
-- }}}
