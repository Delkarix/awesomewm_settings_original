...
local CMD = "grep '^cpu.' /proc/stat" --[[sh -c "grep '^cpu.' /proc/stat; ps -eo 'pid:10,pcpu:5,pmem:5,comm:30,cmd' --sort=-pcpu ]]
...
local function create_textbox(args)
	...
	font = "Terminus 12",
        ...
        markup = args.markup,
        forced_width = args.forced_width or 100,--40,
...
local function worker(user_args)
    ...
    local color = args.color or "#00FF00" --beautiful.fg_normal
    ...
    local popup = awful.popup{
        ...
        border_width = 1,
        border_color = "#ffffff", --beautiful.bg_normal,
        maximum_width = 300,
        offset = { y = 5 },
        widget = {}
    }
    ...
    popup_timer:connect_signal('timeout', function()
        awful.spawn.easy_async(CMD, function(stdout, _, _, _)
			...
			--forced_width = 500,
                ...
                --margins = 20,
                widget = wibox.layout.fixed.horizontal --wibox.container.margin
            }
        end)
    end)
